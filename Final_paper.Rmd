...
```
    ...
    In Figure 4, a negative correlation emerged between GDP per capita and suicide, differentiating between sexes, in all countries. However, looking at this relationship specifically in Europe in Figure 5, we see a general negative correlation between GDP per capita and suicide rate, with dramatic sex differences in suicide rates but the same general trend for both sexes. However, this might be due more to outliers than an actual effect. 
...
```
    ...
    Looking at Figure 6 and 7, we found that the male suicide rate is also higher both across and within continents. Looking closer at Figure 7, suicide rates by continent were the highest in Europe. Female suicide rates are much lower and also have a much shorter tail of outliers.
...
```
    ...
    However, according to Figure 8, by using a faceted graph with a quadratic model, we found that GDP may seem as a negative predictor because of overplotting or the highly variable suicide rate trends across continents. This graph shows that the correlation might not be what it visually appears to be and simple regression models should be explored further.
...
```
    ...
    Using linear regression models to assess the validity of these observations, our main finding was that, across all models, male sex was the most significant and consistent predictor of suicide rates at the 0.0001 level (model 1 and model2).
...
```
    ...
    GDP per capita did also end up being a significant predictor across the entire dataset (at the 0.0001 level), but was not significant in models isolating Europe (model4). 
```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE}
...
```
    ...
    From the exploratory data analysis, Figure 10 indicated that suicide rates have been decreasing overtime for all age groups and there is a large consistent gap in suicide rate among age groups: suicide rate was highest among oldest age groups and lowest among lower age groups. 
...
```
    To see if economic factors influence suicide rates by age/generation, we charted how GDP per capita changed over time for each generation. Figure 12 shows that GDP per capita increased similarly across all generations over time and seems to have no influence on suicide rates for generation groups.
...
ggplot(suimod5, aes(x=resid))+
  geom_freqpoly()+
  labs(title= "Residuals Plot of Suicide Rates with Age, \nContinent, and Sex as Predictors")+
  labs(caption="Figure 15d")#best model based on residuals
summary(mod5)
```
...
title: "Appendix"
---
```{r, message=FALSE, error=FALSE, warning=FALSE, result=}
#-------Read Data--------
library(tidyverse)
library(modelr)
library(ggplot2)
...
Huang, X., Ribeiro, J. D., Musacchio, K. M., & Franklin, J. C. (2017). Demographics as predictors of suicidal thoughts and behaviors: A meta-analysis. PLoS ONE, 12(7). https://doi.org/10.1371/journal.pone.0180793 

Khazaei, S., Armanmehr, V., Nematollahi, S., Rezaeian, S., & Khazaei, S. (2017). Suicide rate in relation to the Human Development Index and other health related factors: A global ecological study from 91 countries—ScienceDirect. Journal of Epidemiology and Global Health, 7(2), 131–134. https://doi.org/10.1016/j.jegh.2016.12.002 

Oyesanya, M., Lopez-Morinigo, J., & Dutta, R. (2015). Systematic review of suicide in economic recession. World Journal of Psychiatry, 5(2), 243–254. https://doi.org/10.5498/wjp.v5.i2.243 

Suicide data. (n.d.). Retrieved December 11, 2020, from https://www.who.int/teams/maternal-newborn-child-adolescent-health-and-ageing/maternal-health/about/mental-health-and-substances-use 

Vijayakumar, L. (2015). Suicide in women. Indian Journal of Psychiatry, 57(Suppl 2), S233–S238. https://doi.org/10.4103/0019-5545.161484 Waern, M., Rubenowitz, E., & 

Wilhelmson, K. (2003). Predictors of Suicide in the Old Elderly. Gerontology, 49, 328–334. https://doi.org/10.1159/000071715